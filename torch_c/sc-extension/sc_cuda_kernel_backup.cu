...
//				int i_point_pack_offset = pack * (i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1) * c_ins * w_w_ins * w_h_ins;
    	...
    	for(int pw=0; pw<bit_packs*w_h_ins; pw++) {
    		...
    		__shared__ int input_s [SHARE_SIZE][BLOCK_SIZE];
    		__shared__ int input_s_1 [SHARE_SIZE][BLOCK_SIZE];
...
//    			__shared__ int input_s_0 [SHARE_SIZE][BLOCK_SIZE];
//    			__shared__ int input_s_1 [SHARE_SIZE][BLOCK_SIZE];
    			for(int i=index_gen; i<inner_size*i_kernel_size; i+=stride_gen) {
...
//    			__shared__ int input_s [SHARE_SIZE][BLOCK_SIZE];
    			for(int i=index_gen; i<inner_size*i_kernel_size; i+=stride_gen) {
    				...
    				int c_in = flatten_in / w_w_ins;
    				int w_in = flatten_in % w_w_ins;
    				if (i_out_size<i_flatten_step) input_s[flatten_in][input_dim] = input_stream_pack[c_in*i_w_ins*i_h_ins + (input_w+w_in)*i_h_ins + (input_h+h_in)];
    			}
    			__syncthreads();
