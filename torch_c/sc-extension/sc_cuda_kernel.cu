...
//					int input_v = i_out_size;
//					int weight_pos_v = flatten_in + w_cout;
//					int weight_neg_v = flatten_in - w_cout;
...
__device__ __forceinline__ void matmul_outer_s(
		const int32_t* __restrict__ input_point,
		const int32_t* __restrict__ weight_pos_stream,
		const int32_t* __restrict__ weight_neg_stream,
		int32_t* __restrict__ output_stream,
	...
	for(int pw=0; pw<pws; pw++) {
		int i_point_pw_offset = pw * i_pw_step;
		int w_pw_offset = pw * w_pw_step;
		int index_i = index_gen%i_kernel_size;
		int index_w = index_gen/i_kernel_size;
		int i_out_size=index_i;
		for(; i_out_size+BLOCK_SIZE-index_i-1<i_flatten_step; i_out_size+=i_kernel_size){
			int w_cout=index_w;
			for(; w_cout+BLOCK_SIZE-index_w-1<w_flatten_step; w_cout+=w_kernel_size) {
				int output_pos_c = 0;
				int output_neg_c = 0;
				int flatten_in_init=0;
				for(; flatten_in_init+BLOCK_SIZE-1<inner_size; flatten_in_init+=BLOCK_SIZE) {
					__shared__ int input_s[BLOCK_SIZE][BLOCK_SIZE+1];
					__shared__ int weight_pos_s[BLOCK_SIZE][BLOCK_SIZE+1];
					__shared__ int weight_neg_s[BLOCK_SIZE][BLOCK_SIZE+1];
					input_s[index_i][index_w] = input_point[i_point_batch_offset + i_point_pw_offset + (flatten_in_init+index_w)*i_flatten_step + i_out_size];
					weight_pos_s[index_w][index_i] = weight_pos_stream[w_pw_offset+(flatten_in_init+index_i)*w_flatten_step + w_cout];
					weight_neg_s[index_w][index_i] = weight_neg_stream[w_pw_offset+(flatten_in_init+index_i)*w_flatten_step + w_cout];
					__syncthreads();
					for(int flatten_in=0; flatten_in<BLOCK_SIZE*2; flatten_in++) {
						int input_v = input_s[index_i][flatten_in];
						int weight_pos_v = weight_pos_s[index_w][flatten_in];
						int weight_neg_v = weight_neg_s[index_w][flatten_in];

						output_pos_c = output_pos_c | (input_v & weight_pos_v);
						output_neg_c = output_neg_c | (input_v & weight_neg_v);
					}
				}
				for(; flatten_in_init<inner_size; flatten_in_init++) {
					int input_v = input_point[i_point_batch_offset + i_point_pw_offset + flatten_in_init*i_flatten_step + i_out_size];
					int weight_pos_v = weight_pos_stream[w_pw_offset+flatten_in_init*w_flatten_step + w_cout];
					int weight_neg_v = weight_neg_stream[w_pw_offset+flatten_in_init*w_flatten_step + w_cout];
			...
			if(w_cout<w_flatten_step) {
				int output_pos_c = 0;
				int output_neg_c = 0;
				for(int flatten_in=0; flatten_in<inner_size; flatten_in++) {
					int input_v = input_point[i_point_batch_offset + i_point_pw_offset + flatten_in*i_flatten_step + i_out_size];
					int weight_pos_v = weight_pos_stream[w_pw_offset+flatten_in*w_flatten_step + w_cout];
					int weight_neg_v = weight_neg_stream[w_pw_offset+flatten_in*w_flatten_step + w_cout];
		...
		for(; i_out_size<i_flatten_step; i_out_size+=i_kernel_size){
			for(int w_cout=index_gen/i_kernel_size; w_cout<w_flatten_step; w_cout+=w_kernel_size) {
				int output_pos_c = 0;
				int output_neg_c = 0;
				for(int flatten_in=0; flatten_in<inner_size; flatten_in++) {
...
__global__
void weight_transpose(
		const int32_t* __restrict__ weight_pos,
		const int32_t* __restrict__ weight_neg,
		int32_t* __restrict__ weight_pos_tran,
		int32_t* __restrict__ weight_neg_tran,
		int c_outs,
		int c_ins,
		int w_ins,
		int h_ins) {
	...
	for(int i=index_gen; i<h_ins * c_ins * w_ins * c_outs; i+=stride_gen) {
		...
		int w_in=(i % (c_outs*w_ins)) / c_outs;
		int c_out=i % c_outs;
		...
		weight_pos_tran[i] = weight_pos_bin;
		weight_neg_tran[i] = weight_neg_bin;
	}
}
...
//			printf("Index %d, i %d, seed %d, pack %d, flatten_in %d\n", index_gen, i, pos_seed_cur, pack, flatten_in);
...
//			printf("Pack %d, weight bin pos %d, value %d, stream %d\n", pack, c_out*c_ins*w_ins*h_ins + c_in*w_ins*h_ins + w_in*h_ins + h_in, weight_pos_bin, weight_pos_stream_c);
//			if (h_in==h_ins-1) {
...
//			printf("Index %d, i %d, seed %d, pack %d, flatten_in %d\n", index_gen, i, pos_seed_cur, pack, flatten_in);
			...
			weight_pos_stream[pack_offset+i] = weight_pos_stream_c;
			weight_neg_stream[pack_offset+i] = weight_neg_stream_c;
//			printf("Stream ind %d, stream pos %d, neg %d, bin pos %d, bin neg %d\n", pack_offset+i, weight_pos_stream_c, weight_neg_stream_c, weight_pos_bin, weight_neg_bin);
		}
//			printf("Pack %d, weight bin pos %d, value %d, stream %d\n", pack, c_out*c_ins*w_ins*h_ins + c_in*w_ins*h_ins + w_in*h_ins + h_in, weight_pos_bin, weight_pos_stream_c);
//			if (h_in==h_ins-1) {
//				pos_seed_arr[flatten_in] = pos_seed_cur;
...
void stream_generation_transpose_add_acc(
	...
	// If sharing, # of seeds = c_ins*w_ins
	if(share){
...
//				if((index_gen==0)||(index_gen==1)) printf("Index %d, Pos %d, neg %d\n", index_gen, rand_num_pos, rand_num_neg);
				...
				total_bit -= 1;
//				printf("Weight_pos, index %d, bit %d, pack %d, bin %d, rand %d\n", i, bit, pack, weight_pos_bin, rand_num_pos);
//				printf("Weight_neg, index %d, bit %d, pack %d, bin %d, rand %d\n", i, bit, pack, weight_neg_bin, rand_num_neg);
			}
			weight_pos_stream[pack_offset+i] = weight_pos_stream_c;
			weight_neg_stream[pack_offset+i] = weight_neg_stream_c;
//			printf("i %d, bin %d, bin_cur %d, stream %d\n", i, weight_pos[c_out*c_ins*w_ins*h_ins + c_in*w_ins*h_ins + w_in*h_ins + h_in], weight_pos_bin, __popc(weight_pos_stream_c));
		}
	}
}
...
void stream_compute_add_acc(
    ...
    // If share, # of unique seeds = c_ins*i_w_ins
...
//				int i_point_pack_offset = pack * (i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1) * c_ins * w_w_ins * w_h_ins;
				for(int bit=0; bit<bit_unit; bit++) {
					...
					total_bit -= 1;
//					printf("Total bit %d, rand %f, input cur %d, bit %d\n", total_bit, input_rand, input_bin_cur, input_bit);
				}
				input_stream[i_stream_batch_offset+i_stream_pack_offset+i] = input_stream_cur;
//				printf("Batch %d, i %d, bin %d, bin_cur %d, stream %d\n", batch, i, input_bin[i_bin_batch_offset+i], input_bin_cur, __popc(input_stream_cur));
        	}
        }
        __syncthreads();
        for(int pack=0; pack<bit_packs; pack++) {
...
//		int* input_point_batch = input_point + i_point_batch_offset;
//		const int* input_bin_batch = input_bin + i_bin_batch_offset;
//		int32_t* output_batch = output_stream + batch*o_batch_step;
//		const int32_t* input_bin_batch = input_bin + batch*i_bin_batch_step;
		int input_seed=0;
...
//		int stride = blockDim.x;
        ...
        for(int i=index_gen; i<c_ins * i_w_ins * i_h_ins; i += stride_gen) {
        	...
        	int input_stream_cur = 0;
        	int input_bin_cur = input_bin[i_bin_batch_offset+i];
//        	int input_bin_cur = input_bin_batch[i];
...
//				input_stream_pack[0] = input_stream_cur;
//				printf("Batch %d, i %d, bin %d, stream %d\n", batch, i, input_bin_cur, __popc(input_stream_cur));
        	}
        }
        __syncthreads();
        for(int pack=0; pack<bit_packs; pack++) {
        	...
        	int i_point_pack_offset = pack * (i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1) * c_ins * w_w_ins * w_h_ins;
//        	int* input_stream_pack = input_stream_batch + i_stream_pack_offset;
//        	int* input_point_pack = input_point_batch + i_point_pack_offset;
			for(int i=index_gen; i<w_h_ins*(i_w_ins-w_w_ins+1)*(i_h_ins-w_h_ins+1)*c_ins*w_w_ins; i+= stride_gen) {
				int h_in = i / ((i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1) * c_ins * w_w_ins);
				int flatten_in = (i % ((i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1) * c_ins * w_w_ins)) / ((i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1));
				int input_cin = flatten_in / w_w_ins;
	...
	//				printf("Stream ind: %d\n", i_stream_pack_offset + input_cin*i_w_ins*i_h_ins + (input_w+input_w_win)*i_h_ins + (input_h+h_in));
				input_point[i_point_batch_offset+i_point_pack_offset+i] = input_stream[i_stream_batch_offset+i_stream_pack_offset + input_cin*i_w_ins*i_h_ins + (input_w+input_w_win)*i_h_ins + (input_h+h_in)];
//				input_point_pack[i] = input_stream_pack[input_cin*i_w_ins*i_h_ins + (input_w+input_w_win)*i_h_ins + (input_h+h_in)];
			}
        }
        __syncthreads();
        matmul_outer(input_point,
...
__global__
void stream_compute_accmult_direct_variable(
		const int32_t* __restrict__ input_bin,
		curandState_t* __restrict__ states_pos,
		curandState_t* __restrict__ states_neg,
		const int32_t* __restrict__ weight_pos_tran,
		const int32_t* __restrict__ weight_neg_tran,
		int32_t* __restrict__ output_stream,
    ...
    curand_init(0, index_gen, 0, states_pos+index_overall);
    curand_init(1, index_gen, 0, states_neg+index_overall);
    for(int batch=index_batch; batch<batches; batch+=stride_batch) {
//    printf("thread ID %d\n", index_gen);
//	for(int batch=0; batch<batches; batch++) {
		int o_batch_offset = batch*o_batch_step;
		int i_bin_batch_offset = batch*i_bin_batch_step;
		int i_stream_batch_offset = index_batch*i_stream_batch_step;
		int* output_batch = output_stream + o_batch_offset;
		const int* input_bin_batch = input_bin + i_bin_batch_offset;
		int input_dim = index_gen%i_kernel_size;
		int weight_dim = index_gen/i_kernel_size;
    	// Input stationary
    	for(int h_in=0; h_in<w_h_ins; h_in++) {
    		const int* weight_pos_hin = weight_pos_tran + h_in*w_pw_step;
    		const int* weight_neg_hin = weight_neg_tran + h_in*w_pw_step;
//    		if((index_batch==0) && (index_gen==0)) printf("h_in %d, batch %d, index_gen %d\n", h_in, index_batch, index_gen);
			// Process the remaining input grid
			for(int i_out_size=input_dim; i_out_size<i_flatten_step; i_out_size+=i_kernel_size){
				...
				for(int w_cout=weight_dim; w_cout<w_flatten_step; w_cout+=w_kernel_size) {
					for(int bit_pack=0; bit_pack<bit_packs; bit_pack++) {
						int output_pos_c = 0;
						int output_neg_c = 0;
						for(int flatten_in=0; flatten_in<inner_size; flatten_in++) {
							int pos_v;
							int neg_v;
							if(bit_pack==0){
								int c_in = flatten_in / w_w_ins;
								int w_in = flatten_in % w_w_ins;
								int input_v = input_bin_batch[c_in*i_w_ins*i_h_ins + (input_w+w_in)*i_h_ins + (input_h+h_in)];
								int weight_pos_v = weight_pos_hin[flatten_in*w_flatten_step + w_cout];
								int weight_neg_v = weight_neg_hin[flatten_in*w_flatten_step + w_cout];

								pos_v = (input_v * weight_pos_v + bit_length-1) / bit_length;
								neg_v = (input_v * weight_neg_v + bit_length-1) / bit_length;
							}
							else {
								pos_v = output_pos[flatten_in*stride_gen+input_dim*w_kernel_size+weight_dim];
								neg_v = output_neg[flatten_in*stride_gen+input_dim*w_kernel_size+weight_dim];
							}
//							printf("pos_v %d, neg_v %d\n", pos_v, neg_v);

							int pos_c = 0;
							int neg_c = 0;

							int total_bit = (bit_packs-bit_pack)*bit_unit;
							for(int bit=0; bit<bit_unit; bit++) {
								int rand_num_pos = curand_uniform(states_pos+index_overall) * total_bit;
								int rand_num_neg = curand_uniform(states_neg+index_overall) * total_bit;
								int pos_bit = pos_v > rand_num_pos;
								int neg_bit = neg_v > rand_num_neg;
								pos_c = pos_c*2 + pos_bit;
								pos_v -= pos_bit;
								neg_c = neg_c*2 + neg_bit;
								neg_v -= neg_bit;
								total_bit --;
							}
							output_pos_c |= pos_c;
							output_neg_c |= neg_c;
							output_pos[flatten_in*stride_gen+input_dim*w_kernel_size+weight_dim] = pos_v;
							output_neg[flatten_in*stride_gen+input_dim*w_kernel_size+weight_dim] = neg_v;
						}
						output_batch[i_out_size+o_cout_step*w_cout] += __popc(output_pos_c) - __popc(output_neg_c);
					}
				}
			}
    	}
    		...
    		for(int flatten_in_cur=0; flatten_in_cur<inner_size; flatten_in_cur+=SHARE_SIZE) {
				...
				for(; i_out_size-input_dim+2*i_kernel_size-1<i_flatten_step; i_out_size+=2*i_kernel_size) {
					...
					for(int i=index_gen; i<SHARE_SIZE*i_kernel_size; i+=stride_gen) {
						...
						int w_in = (flatten_in + flatten_in_cur) % w_w_ins;
						if (flatten_in + flatten_in_cur<inner_size) {
//							input_s[flatten_in*i_kernel_size+input_dim] = input_stream_pack[c_in*i_w_ins*i_h_ins + (input_w+w_in)*i_h_ins + (input_h+h_in)];
//							input_s[(flatten_in+SHARE_SIZE)*i_kernel_size+input_dim] = input_stream_pack[c_in*i_w_ins*i_h_ins + (input_w_1+w_in)*i_h_ins + (input_h_1+h_in)];
							input_s[flatten_in][input_dim] = input_stream_pack[c_in*i_w_ins*i_h_ins + (input_w+w_in)*i_h_ins + (input_h+h_in)];
							input_s_1[flatten_in][input_dim] = input_stream_pack[c_in*i_w_ins*i_h_ins + (input_w_1+w_in)*i_h_ins + (input_h_1+h_in)];
						}
					}
					...
					for(int w_cout=weight_dim; w_cout<w_flatten_step; w_cout+=w_kernel_size) {
						...
						for(int flatten_in=0; flatten_in<SHARE_SIZE; flatten_in++) {
							if (flatten_in + flatten_in_cur<inner_size) {
								int input_v = input_s[flatten_in][input_dim];
//								int input_v = input_s[flatten_in*i_kernel_size+input_dim];
								int weight_pos_v = weight_pos_pw[(flatten_in+flatten_in_cur)*w_flatten_step + w_cout];
								int weight_neg_v = weight_neg_pw[(flatten_in+flatten_in_cur)*w_flatten_step + w_cout];
								output_pos_c |= (input_v & weight_pos_v);
								output_neg_c |= (input_v & weight_neg_v);
								...
								int input_v_1 = input_s_1[flatten_in][input_dim];
//								int input_v_1 = input_s[(flatten_in+SHARE_SIZE)*i_kernel_size+input_dim];
								output_pos_c_1 |= (input_v_1 & weight_pos_v);
								output_neg_c_1 |= (input_v_1 & weight_neg_v);
							}
						}
						...
						output_neg[o_batch_offset+i_out_size+i_kernel_size+o_cout_step*w_cout] |= output_neg_c_1;
//						output_stream[o_batch_offset+i_out_size+o_cout_step*w_cout] += __popc(output_pos_c) - __popc(output_neg_c);
//						output_stream[o_batch_offset+i_out_size+i_kernel_size+o_cout_step*w_cout] += __popc(output_pos_c_1) - __popc(output_neg_c_1);
					}
					__syncthreads();
				}
				// Process the remaining input grid
...
//						if ((i_out_size<i_flatten_step) && (flatten_in+flatten_in_cur<inner_size)) input_s[flatten_in*i_kernel_size + input_dim] = input_stream_pack[c_in*i_w_ins*i_h_ins + (input_w+w_in)*i_h_ins + (input_h+h_in)];
					}
					__syncthreads();
...
//									int input_v = input_s[flatten_in*i_kernel_size + input_dim];
							...
							output_pos[o_batch_offset+i_out_size+o_cout_step*w_cout] |= output_pos_c;
							output_neg[o_batch_offset+i_out_size+o_cout_step*w_cout] |= output_neg_c;
//							printf("output_pos %d, output_neg %d, flatten_in_cur %d, inner_size %d, share_size %d\n", __popc(output_pos[o_batch_offset+i_out_size+o_cout_step*w_cout]), __popc(output_neg[o_batch_offset+i_out_size+o_cout_step*w_cout]), flatten_in_cur,
//									inner_size, SHARE_SIZE);
//							output_stream[o_batch_offset+i_out_size+o_cout_step*w_cout] += __popc(output_pos_c) - __popc(output_neg_c);
						}
					}
					__syncthreads();
		...
		for(int pw=0; pw<bit_packs*w_h_ins; pw++) {
			...
			for(; i_out_size+BLOCK_SIZE-index_i-1<i_flatten_step; i_out_size+=i_kernel_size){
				...
				for(; w_cout+BLOCK_SIZE-index_w-1<w_flatten_step; w_cout+=w_kernel_size) {
					...
					int output_neg_c = 0;
					int flatten_in_init=0;
					for(; flatten_in_init+BLOCK_SMALL-1<inner_size; flatten_in_init+=BLOCK_SMALL) {
						__shared__ int input_s[BLOCK_SIZE][BLOCK_SMALL+1];
						__shared__ int weight_pos_s[BLOCK_SIZE][BLOCK_SMALL+1];
						__shared__ int weight_neg_s[BLOCK_SIZE][BLOCK_SMALL+1];
						if (index_w<BLOCK_SMALL) {
							input_s[index_i][index_w] = input_point[i_point_batch_offset + i_point_pw_offset + (flatten_in_init+index_w)*i_flatten_step + i_out_size];
						}
						if (index_i<BLOCK_SMALL) {
							weight_pos_s[index_w][index_i] = weight_pos_stream[w_pw_offset+(flatten_in_init+index_i)*w_flatten_step + w_cout];
							weight_neg_s[index_w][index_i] = weight_neg_stream[w_pw_offset+(flatten_in_init+index_i)*w_flatten_step + w_cout];
						}

						__syncthreads();
						for(int flatten_in=0; flatten_in<BLOCK_SMALL; flatten_in++) {
							int input_v = input_s[index_i][flatten_in];
							int weight_pos_v = weight_pos_s[index_w][flatten_in];
							int weight_neg_v = weight_neg_s[index_w][flatten_in];
...
//		const int32_t* input_bin_batch = input_bin + batch*i_bin_batch_step;
        for(int i=index_gen; i<z_packs * z_units * i_w_ins * i_h_ins; i += stride_gen) {
        	...
        	int input_bin_actual;
        	int cur_bit=0;
//        	printf("i %d, c_in %d, flatten %d, c_in %d, i_w_ins %d, i_h_ins %d\n", i, c_in, flatten, c_ins*i_w_ins*i_h_ins,i_w_ins,i_h_ins);
//        	printf("i %d, bin_cur %d\n", i, input_bin_cur);
...
//				input_point[i_point_batch_offset+i_point_pack_offset+i] = 1;
...
//				printf("Point ind: %d, stream ind %d, %d, %d, %d, %d, value %d\n", i_point_batch_offset+i_point_pack_offset+i,
////						w_h_ins*z_units*w_w_ins,
//						i, w_h_ins*z_units,w_w_ins*cin_size,
////						i_stream_batch_offset+i_stream_pack_offset + c_in*i_w_ins*i_h_ins + (input_w+w_in)*i_h_ins + (input_h+h_in),
//						input_point[i_point_batch_offset+i_point_pack_offset+i]);
			}
        }
        __syncthreads();
...
//    if (index_batch==0) curand_init(seed, index_gen, 0, &states[index_gen]);
//    __syncthreads();
    for(int batch=index_batch; batch<batches; batch+=stride_batch) {
//    printf("thread ID %d\n", index_gen);
//	for(int batch=0; batch<batches; batch++) {
...
//        printf("Something id %d\n", index_gen);
		for(int i=index_gen; i<w_h_ins*(i_w_ins-w_w_ins+1)*(i_h_ins-w_h_ins+1)*c_ins*w_w_ins; i+= stride_gen) {
			int h_in = i / ((i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1) * c_ins * w_w_ins);
//				int flatten_in = (i % ((i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1) * c_ins * w_w_ins)) % (c_ins*w_w_ins);
			int flatten_in = (i % ((i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1) * c_ins * w_w_ins)) / ((i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1));
			int input_cin = flatten_in / w_w_ins;
			int input_w_win = flatten_in % w_w_ins;
			int flatten = i % ((i_w_ins-w_w_ins+1) * (i_h_ins-w_h_ins+1));
...
void stream_compute_or_im2col(
    ...
    int index_overall = blockIdx.x*gridDim.x + threadIdx.x;
//    curandState_t* state;
//    curand_init(seed, index_gen, 0, state);
//    printf("Something id %d\n", index_gen);
//    for(int index_cur=index_overall; index_cur<inner_size*cin_size; index_cur+=stride_gen*stride_batch) {
//    	curand_init(seed, index_cur, 0, states+index_cur);
//    }
    curand_init(seed, index_overall, 0, states+index_overall);
//    __syncthreads();
//    curand_init(seed, index_gen, 0, &states[index_gen]);
//    if ((index_gen==0) && (index_batch==0)) printf("Before loop\n");
    for(int batch=index_batch; batch<batches; batch+=stride_batch) {
//    printf("thread ID %d\n", index_gen);
//	for(int batch=0; batch<batches; batch++) {
		int o_batch_offset = batch*o_batch_step;
...
//        printf("Something id %d\n", index_gen);
		...
		for(int i=index_gen; i<(i_w_ins-w_w_ins+1)*(i_h_ins-w_h_ins+1)*c_ins*w_w_ins*w_h_ins; i+= stride_gen) {
			...
			for(int pack=0; pack<bit_packs; pack++) {
				...
				for(int bit=0; bit<bit_unit; bit++) {
					input_rand = curand(states + index_overall) % bit_length;
//					input_rand = curand(states + index_batch*stride_gen + index_gen) % bit_length;
					input_stream_cur = (input_stream_cur*2) + (input_bin_cur > input_rand);
				}
...
//	cudaFuncSetAttribute(stream_compute_add_direct_variable, cudaFuncAttributeMaxDynamicSharedMemorySize, maxbytes);
...
//	return torch::zeros({input_size[0], weight_size[0], (input_size[2]-weight_size[2]+1), (input_size[3]-weight_size[3]+1)}, at::TensorOptions().dtype(torch::kInt32).device(device));
}

torch::Tensor conv2d_add_partial_accmult_variable_cuda(torch::Tensor input,
		torch::Tensor weight_pos,
		torch::Tensor weight_neg,
		int bit_length,
		int total_width,
	...
	int32_t *weight_pos_tran, *weight_neg_tran;
	cudaMallocManaged(&weight_pos_tran, weight_size[3]*weight_size[1]*weight_size[2]*weight_size[0]*sizeof(int32_t));
	cudaMallocManaged(&weight_neg_tran, weight_size[3]*weight_size[1]*weight_size[2]*weight_size[0]*sizeof(int32_t));

	const int threads = 1024;
	const int numBlocks_gen = (weight_size[0]*weight_size[1]*weight_size[2]*weight_size[3] + threads -1)/threads;
	int pos_seed = 67;
	int neg_seed = 37;
	weight_transpose<<<numBlocks_gen, threads>>>(
			weight_pos.data_ptr<int32_t>(),
			weight_neg.data_ptr<int32_t>(),
			weight_pos_tran,
			weight_neg_tran,
	...
	int w_kernel = 16;
	const int threads_c = i_kernel*w_kernel;
	curandState_t *states_pos, *states_neg;
	cudaMallocManaged(&states_pos, threads_c*numBlocks_comp * sizeof(curandState_t));
	cudaMallocManaged(&states_neg, threads_c*numBlocks_comp * sizeof(curandState_t));


	int32_t* output_point = output_tensor.data_ptr<int32_t>();
	int32_t *output_pos, *output_neg;
	cudaMallocManaged(&output_pos, inner_size*threads_c*sizeof(int32_t));
	cudaMallocManaged(&output_neg, inner_size*threads_c*sizeof(int32_t));
	stream_compute_accmult_direct_variable<<<numBlocks_comp, threads_c>>>(
			input.data_ptr<int32_t>(),
			states_pos,
			states_neg,
			weight_pos_tran,
...
//	cudaDeviceSynchronize();
	cudaFree(weight_pos_tran);
	cudaFree(weight_neg_tran);
	cudaFree(states_pos);
	cudaFree(states_neg);
	cudaFree(output_pos);
	cudaFree(output_neg);
...
//	cudaDeviceSynchronize();
	auto output_tensor = torch::zeros({input_size[0], weight_size[0], (input_size[2]-weight_size[2]+1), (input_size[3]-weight_size[3]+1)}, at::TensorOptions().dtype(torch::kInt32).device(device));
//	auto input_test = torch::zeros({1}, at::TensorOptions().dtype(torch::kInt32).device(device));
//	int32_t* input_bin_point = input.data_ptr<int32_t>();
    int o_batch_step = weight_size[0] * (input_size[2]-weight_size[2]+1) * (input_size[3]-weight_size[3]+1);
    int i_bin_batch_step = input_size[1] * input_size[2] * input_size[3];
    int add_size = bit_packs * weight_size[3];
    int inner_size = weight_size[1] * weight_size[2];
...
torch::Tensor conv2d_add_partial_variable_cuda(torch::Tensor input,
		torch::Tensor weight_pos,
		torch::Tensor weight_neg,
		int bit_length,
		int total_width,
	...
	cudaMallocManaged(&weight_pos_stream, bit_packs*weight_size[3]*weight_size[1]*weight_size[2]*weight_size[0]*sizeof(int32_t));
	cudaMallocManaged(&weight_neg_stream, bit_packs*weight_size[3]*weight_size[1]*weight_size[2]*weight_size[0]*sizeof(int32_t));
//	auto weight_pos_stream = torch::zeros({bit_packs, weight_size[3], weight_size[1], weight_size[2], weight_size[0]}, at::TensorOptions().dtype(torch::kInt32).device(device));
//	auto weight_neg_stream = torch::zeros({bit_packs, weight_size[3], weight_size[1], weight_size[2], weight_size[0]}, at::TensorOptions().dtype(torch::kInt32).device(device));
	int weight_stream_size = bit_packs*weight_size[0]*weight_size[1]*weight_size[2]*weight_size[3];

//	const int threads = 1024;
//	const int numBlocks = (weight_stream_size + threads - 1) / threads;
	const int threads = 1024;
//	const int numBlocks_gen = 1;
	const int numBlocks_gen = (weight_size[0]*weight_size[1]*weight_size[2]*weight_size[3] + threads -1)/threads;
	int *pos_seed_arr, *neg_seed_arr;
	cudaMallocManaged(&pos_seed_arr, weight_size[1]*weight_size[2]*sizeof(int32_t));
	cudaMallocManaged(&neg_seed_arr, weight_size[1]*weight_size[2]*sizeof(int32_t));
	...
	stream_generation_transpose_add_variable<<<numBlocks_gen, threads>>>(
			...
			weight_pos_stream,
			weight_neg_stream,
//			weight_pos_stream.data_ptr<int32_t>(),
//			weight_neg_stream.data_ptr<int32_t>(),
			pos_seed_arr,
			neg_seed_arr,
			bit_length,
			weight_size[0],
...
//	int i_kernel = BLOCK_SIZE;
//	int w_kernel = BLOCK_SIZE;
//	const int threads_c = i_kernel * w_kernel;
...
//		for(int cur_batch=0; cur_batch<input_size[0]; cur_batch+=numBlocks_comp) {
//			stream_compute_transpose_variable<<<numBlocks_comp, threads_c>>>(
//					input.data_ptr<int32_t>(),
//					input_seed_arr,
//					input_stream,
//					input_point,
...
//			stream_add_variable<<<numBlocks_comp, threads_c>>>(
//					input_point,
//					weight_pos_stream,
//					weight_neg_stream,
//					output_point,
...
//	for(int i=0; i<weight_size[2] * weight_size[3]; i++) {
//		pos_seed_arr[i] = (pos_seed + i)%(bit_length-1) + 1;
//		neg_seed_arr[i] = (neg_seed + i)%(bit_length-1) + 1;
//	}
...
//	auto weight_pos_stream = torch::zeros({bit_packs, weight_size[3], weight_size[1], weight_size[2], weight_size[0]}, at::TensorOptions().dtype(torch::kInt32).device(device));
//	auto weight_neg_stream = torch::zeros({bit_packs, weight_size[3], weight_size[1], weight_size[2], weight_size[0]}, at::TensorOptions().dtype(torch::kInt32).device(device));
//	int weight_stream_size = bit_packs*weight_size_total;
