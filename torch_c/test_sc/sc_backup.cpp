...
//     start = clock();
    ...
    std::cout << input_size[1] << " " << add_full << std::endl;
    std::cout << split_size << std::endl;
    auto input_split = at::split(input, split_size, -1);
    std::cout << input_split << std::endl;
    ...
    auto rand_input_test = input_split;
    for(int i=0; i<add_full; i++) {
        rand_input_test[i].uniform_();
    }
    std::cout << rand_input_test << std::endl;
    
    auto input_bit = (input > rand_input).to(torch::kUInt8);
    auto weight_pos_bit = (weight_pos > rand_weight_pos).to(torch::kUInt8);
    auto weight_neg_bit = (weight_neg > rand_weight_neg).to(torch::kUInt8);
...
//     start = clock();
    for(int i=1; i<bit_length; i++) {
        ...
        input_bit = input > rand_input;
        weight_pos_bit = weight_pos > rand_weight_pos;
        weight_neg_bit = weight_neg > rand_weight_neg;
